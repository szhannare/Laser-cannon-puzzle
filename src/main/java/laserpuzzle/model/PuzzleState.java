...
public class PuzzleState implements State<Direction> {
    ...
    /**
     * Creates a {@code PuzzleState} object initializing the positions of the pieces with the positions specified. The constructor expects an array of four {@code Position} objects or four {@code Position} objects.
     *
     * @param position the initial position of the player
     * @param turn the turn which the player is currently in
     */
    public PuzzleState(Position position, int turn) {
    ...
    /**
     * Checks if the player is given a viable position when a {@code PuzzleState} is instantiated
    ...
    public boolean isSolved() {
        return (this.position.col()==14 && this.position.row()==14);
    }
    ...
    private boolean blockedSquare(){
        if ((position.col() == 6 & position.row() == 14)){
            return true;
        } else return position.col() == 13 & position.row() == 13;
    }
    ...
    /**
     * {@return the deepcloned {@code PuzzleState} object /the object itself, not just the reference to it/}
     */
    @Override
